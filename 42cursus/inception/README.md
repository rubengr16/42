...
The current project is an introduction to Docker and its different commands. This includes [Dockerfiles](https://docs.docker.com/reference/dockerfile/) and [docker compose files](https://docs.docker.com/reference/compose-file/).

Regarding Dockerfiles, all services will be created from scratch using the current latest version of [Alpine available in Dockerhub](https://hub.docker.com/layers/library/alpine/3.23.2/images/sha256-16ff8a639f58b38d94b054e94c106dbbd8a60d45f8b1989f98516a3e8e0792ad) -alpine:3.32.3 on the 1st of January, 2026-.

Services said above are a mariadb, wordpress and nginx.

Mariadb will store all the information generated by the wordpress
application. This container will have a volume mapped at
`/home/\<login\>/data/mariadb` -in this case is `rgallego`- and exposes
internally the port 3306.

The wordpress container uses this port. It has installed php83 and
php-fpm which work as CGI -Common Gateway Interface-. Along this
applications worpress in its CLI version is installed and after it
successful installation an admin and a normal users are created. Here
another volume is used `/home/\<login\>/data/html` to store all the wordpress
files and share them with the NGINX container. Finally the 9000 is
exposed internally to receive the CGI requests redirected from NGINX.

The third service is nginx, which is configured to map correctly
the requests received through the port 443 -SSL configured with the keys
generated- and return the information obtain from PHP and wordpress.
...
| Includes OS | Yes -full guest OS- | No -shares host kernel- |
| Startup time | Minutes | Seconds |
| Resource usage | Heavy | Lightweight |
| Isolation | Strong | Process-level -lighter- |
| Portability | Limited | Very high |
| Typical use | Running multiple OSes | Microservices, CI/CD, apps |
...
| - | - | - |
| Purpose | Store sensitive data securely | Configure app behavior |
| Security | Encrypted / restricted access | Plaintext |
| Visibility | Not exposed in process list | Visible via `env` |
| Best for | Passwords, tokens, keys | App settings, flags |
| Docker support | [`docker secret`](https://docs.docker.com/reference/cli/docker/secret/) / [`secrets:` in docker compose file](https://docs.docker.com/reference/compose-file/secrets/) / [Build secrets](https://docs.docker.com/build/building/secrets/) | [Dockerfile ARG](https://docs.docker.com/reference/dockerfile/#arg) / [`environment:` in docker compose file](https://docs.docker.com/reference/compose-file/services/#environment) / [Build variables](https://docs.docker.com/build/building/variables/) |
| Risk level | Low (if used correctly) | High for sensitive data |

### Docker Network vs Host Network

| Aspect| [Docker Network](https://docs.docker.com/reference/compose-file/networks/) | Host Network |
| - | - | - |
| Network isolation | Isolated | None |
| Container IP | Yes | Uses host IP |
| Port mapping | Required | Not needed |
| Security | Safer | Riskier |
| Performance | Slight overhead | Max performance |
| Cross-container DNS | Built-in | Not available |
| Typical use | Most apps | Low-latency / debugging |

### Docker Volumes vs Bind Mounts
| Aspect | [Docker Volumes](https://docs.docker.com/reference/compose-file/volumes/) | Bind Mounts |
| - | - | - |
| Managed by Docker | Yes | No |
| Location | Docker-controlled | Any host path |
| Portability | High | Low |
| Performance | Optimized | OS-dependent |
| Security | Better isolation | Full host access |
| Backup | Easy | Manual |
| Best for | Persistent app data | Local development |
| Example | Named volume: toplevel `volumes: \n\wordpress-html:` especification and later use in `services: \n\t nginx: \n\t\t volumes: \n\t\t\t - wordpress-html:/var/www/html` | Path mapping: `./requirements/nginx/conf/nginx.conf:/etc/nginx/nginx.conf` |
...
### mariadb-server.cnf - Bind mount
`skip-networking` will be commented and `user` and `data` will be hardcoded to `mysql` and `/data` -the real target of the alias for the symlink
`/var/lib/mysql`- which are necessary to work with MariaDB. By setting those parameters we don't need to use flags like `--user` and `--datadir` on the MariaDB programs.  

Finally, `bind-address` is set to `0.0.0.0`.

### Entrypoint
* `init.sh`: `mariadb-install-db  --skip-test-db` creates and initializes the DB, the `skip-test-db` option avoids the creation of an unwanted `test` DB
* `mariadbd --bootstrap --skip-grant-tables=false`: MariaDB is set for wordpress using again `cat << EOF` to substitute variables that will be passed to the MariaDB command.  
...
*`mariadbd-safe`: starts safely the MariaDB daemon.

## NGINX
### nginx.conf - Bind mount
The mail NGINX config file is located in `/etc/nginx/nginx.conf` which can include other files in `/etc/nginx/http.d/*.conf`, but we decide to change the first one as it is the main.

The server_name is the login + `.42` + `.fr`: `rgallego.42.fr`.
...
### ENTRYPOINT
* Generate the RSA:2048 keys: use `openssl req` to create a RSA:2048 using flags like `-newkey` to indicate the action of generating a new key, among others. To bring up some flags: indicate the out file for storing the private key with `-keyout` and `out` to indicate the path of the public certificate. Finally the `-subj` gives some information about the `/O` organization, `/OU` organizational unit, `/CN` , `/C` country and `/ST` state, this information is not necessary.
* Substitute the SERVER_NAME
* `nginx` daemon

## Wordpress + PHP-fpm
### www.conf - Bind mount
We set the user and group to nobody and nogroup respectively and to listen in 9000 or 0.0.0.0:9000 which is the same.

### wp-config - Bind mount
Secrets and env variables are obtained and substituted in order to let PHP and wordpress connect with MariaDB
as well as other security procedures.

### ENTRYPOINT
* `php -d memory_limit=512M /usr/local/bin/wp core download `: downloads WP-CLI, runs with PHP and editing the memory limit to avoid the cap of maximun download
* `wp core install`: installs WP-CLI with the given URL, TITLE and admin user
* `wp user create`: creates a normal user
* `php-fpm83 --nodaemonize`: starts php-fpm
...
Once all the VM is setup, the Makefile has several commands:
...
* [WP-CLI installation](https://wp-cli.org/es/)
